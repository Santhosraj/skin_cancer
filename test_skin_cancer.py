...
class HybridEnsemble:
    def __init__(self, input_shape, num_classes=2, load_checkpoint=False, checkpoint_path=None):
...
class SubcategoryModel(nn.Module):
    def __init__(self, num_classes, load_checkpoint=False, checkpoint_path=None):
        ...
        if load_checkpoint and checkpoint_path and os.path.exists(checkpoint_path):
            ...
            print(f"Loaded subcategory model from {checkpoint_path}")
...
if __name__ == '__main__':
    try:
        ...
        # Step 5: Train or Load Binary Classifier
        print("\n5. Training or Loading Binary Hybrid Ensemble...")
        input_shape = (3, 224, 224)
        if os.path.exists(binary_checkpoint_path) and os.path.exists(cnn_checkpoint_path):
            binary_model = HybridEnsemble(input_shape=input_shape, num_classes=2, load_checkpoint=True, checkpoint_path=binary_checkpoint_path)
        else:
        ...
        # Step 6: Train or Load Subcategory Models
        print("\n6. Training or Loading Benign Subcategory Model...")
        if os.path.exists(benign_checkpoint_path):
            benign_model = SubcategoryModel(num_classes=len(benign_classes), load_checkpoint=True, checkpoint_path=benign_checkpoint_path)
        else:
        ...
        print("\n7. Training or Loading Malignant Subcategory Model...")
        if os.path.exists(malignant_checkpoint_path):
            malignant_model = SubcategoryModel(num_classes=len(malignant_classes), load_checkpoint=True, checkpoint_path=malignant_checkpoint_path)
        else:
