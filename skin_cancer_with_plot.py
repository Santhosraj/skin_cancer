...
checkpoint_dir = Path(r'D:\college - Sri eshwar\sumathi mam\checkpoints')
checkpoint_dir.mkdir(exist_ok=True)
selected_features_path = checkpoint_dir / 'selected_features.pkl'
...
try:
    ...
    print("Binary Classes:", binary_train_dataset.classes)
...
binary_train_subset, binary_val_subset = torch.utils.data.random_split(binary_train_dataset, [train_len, test_len])
...
class LightweightCNN(nn.Module):
    ...
    def forward(self, x):
        if x.dim() == 5:
            ...
            raise ValueError(f"Expected 4D input, got {x.dim()}D input of shape {x.shape}")
...
class EnsemblePSO:
    ...
    def _evaluate_fitness(self, weights, predictions, true_labels):
        ...
        return balanced_accuracy_score(true_labels, ensemble_pred)
...
class HybridEnsemble:
    ...
    def _train_cnn(self, train_loader, val_loader, epochs=100):
    ...
    def fit(self, X, y, X_val=None, y_val=None, cnn_loader=None, val_loader=None, feature_names=None, selected_features=None):
        ...
        print("Training CNN...")
        start_time = time.time()
        self._train_cnn(cnn_loader, val_loader)
        ...
        pso = EnsemblePSO()
        self.ensemble_weights, ensemble_fitness = pso.optimize([svm_pred, rf_pred, cnn_pred], y_val)
        ...
        with torch.no_grad():
            for inputs, _ in loader:
                ...
                if inputs.dim() == 5:
                    ...
                    raise ValueError(f"Expected 4D input, got {inputs.dim()}D input of shape {inputs.shape}")
                print(f"Input shape to CNN in predict: {inputs.shape}")
                outputs = torch.softmax(self.cnn(inputs), dim=1)
                cnn_pred.extend(outputs[:, 1].cpu().numpy())
        cnn_pred = np.array(cnn_pred)
    ...
    def load(self, filepath):
        ...
        self.svm_train_time = checkpoint.get('svm_train_time', 0)
        self.rf_train_time = checkpoint.get('rf_train_time', 0)
        self.cnn_train_time = checkpoint.get('cnn_train_time', 0)
...
def compute_metrics(y_true, y_pred, y_prob, model_name):
    ...
    return {'Accuracy': accuracy, 'Precision': precision, 'Recall': recall, 'F1-Score': f1, 'AUC-ROC': auc_roc, 'Confusion Matrix': cm}

def plot_individual_metrics(model_name, metrics):
    metrics_names = ['Accuracy', 'Precision', 'Recall', 'F1-Score', 'AUC-ROC']
    colors = ['blue', 'green', 'orange', 'red', 'purple']
    fig, ax = plt.subplots(figsize=(8, 6))
    x = list(range(len(metrics_names)))  # One x-point per metric
    
    for i, metric in enumerate(metrics_names):
        ax.plot([x[i]], [metrics[metric]], '-o', color=colors[i], label=metric)
        ax.text(x[i], metrics[metric] + 0.02, f'{metrics[metric]:.4f}', ha='center', va='bottom', color=colors[i])
...
def plot_comparative_metrics(all_metrics):
    ...
    x = list(range(1, len(model_names) + 1))
    
    for metric in metrics_names:
        fig, ax = plt.subplots(figsize=(10, 6))
        metric_values = [all_metrics[model_name][metric] for model_name in model_names]
        
        # Plot segments with different colors
        for idx in range(len(model_names) - 1):
            ax.plot(x[idx:idx+2], metric_values[idx:idx+2], '-o', color=colors[idx % len(colors)], label=model_names[idx] if idx == 0 else "")
            ax.text(x[idx], metric_values[idx] + 0.02, f'{metric_values[idx]:.4f}', ha='center', va='bottom', color=colors[idx % len(colors)])
        # Add the last point's text
        ax.text(x[-1], metric_values[-1] + 0.02, f'{metric_values[-1]:.4f}', ha='center', va='bottom', color=colors[(len(model_names) - 1) % len(colors)])
        ...
        ax.grid(True, alpha=0.3)
        ax.legend(model_names)  # Explicitly set legend to model names
        plt.tight_layout()
        plt.show()
...
def classify_image(binary_model, benign_model, malignant_model, image_path, extractor):
    ...
    X_new_full = np.array([feature_vector])
    
    if binary_model.selected_features is None:
        raise ValueError("Selected features not set in the model. Please load a model with selected features.")
    selected_indices = [feature_names.index(f) for f in binary_model.selected_features if f in feature_names]
    X_new = X_new_full[:, selected_indices]
    
    loader = DataLoader([(image_tensor, 0)], batch_size=1)
    binary_pred, binary_prob = binary_model.predict(X_new, loader=loader)
...
if __name__ == '__main__':
    try:
        ...
        print("\n4. Loading Selected Features...")
        if os.path.exists(selected_features_path):
            with open(selected_features_path, 'rb') as f:
                selected_features = pickle.load(f)
            print(f"Loaded selected features: {len(selected_features)} features")
        else:
            raise FileNotFoundError("Selected features file not found. Please run the training script first.")
        ...
        # Step 5: Load Binary Classifier
        print("\n5. Loading Binary Hybrid Ensemble...")
        input_shape = (3, 224, 224)
        if os.path.exists(binary_checkpoint_path) and os.path.exists(cnn_checkpoint_path):
            binary_model = HybridEnsemble(input_shape=input_shape, num_classes=2)
            binary_model.load(binary_checkpoint_path)
            binary_model.cnn.load_state_dict(torch.load(cnn_checkpoint_path))
            binary_model.cnn.eval()
            print("Binary model loaded successfully.")
        else:
            raise FileNotFoundError("Binary model checkpoints not found. Please train the model first.")

        # Step 6: Load Subcategory Models
        print("\n6. Loading Benign Subcategory Model...")
        if os.path.exists(benign_checkpoint_path):
            benign_model = SubcategoryModel(num_classes=len(benign_classes))
            benign_model.model.load_state_dict(torch.load(benign_checkpoint_path))
            benign_model.model.eval()
            print("Benign model loaded successfully.")
        else:
            raise FileNotFoundError("Benign model checkpoint not found. Please train the model first.")

        print("\n7. Loading Malignant Subcategory Model...")
        if os.path.exists(malignant_checkpoint_path):
            malignant_model = SubcategoryModel(num_classes=len(malignant_classes))
            malignant_model.model.load_state_dict(torch.load(malignant_checkpoint_path))
            malignant_model.model.eval()
            print("Malignant model loaded successfully.")
        else:
            raise FileNotFoundError("Malignant model checkpoint not found. Please train the model first.")

        # Step 8: Benchmarking and Validation
        print("\n8. Performing Benchmarking and Validation...")
        
        # Train XGBoost (since no checkpoint is assumed)
        ...
        # Train ResNet-50 (since no checkpoint is assumed)
        ...
        for name, (model, data) in models.items():
            ...
            all_metrics[name] = compute_metrics(y_val, pred, prob, name)
            plot_individual_metrics(name, all_metrics[name])

        # Comparative Analysis Graphs
        print("\nPlotting Comparative Analysis...")
        plot_comparative_metrics(all_metrics)

        # Example classification
        print("\n9. Classifying an Example Image...")
