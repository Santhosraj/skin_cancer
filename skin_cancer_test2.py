...
# Dataset paths
base_data_path = Path("D:\college - Sri eshwar\sumathi mam\datasets\skin_cancer1\Skin cancer ISIC The International Skin Imaging Collaboration")  # Update this to your actual dataset path
...
class HybridEnsemble:
    ...
    def _train_cnn(self, train_loader, val_loader, epochs=50):
    ...
    def fit(self, X, y, X_val=None, y_val=None, cnn_loader=None, val_loader=None):
...
def train_resnet(train_loader, val_loader, num_classes=2, epochs=10):
...
if __name__ == '__main__':
    try:
        ...
        print("\n5. Training Binary Hybrid Ensemble...")
        input_shape = (3, 224, 224)
        binary_model = HybridEnsemble(input_shape=input_shape, num_classes=2)
        binary_model.fit(X_train_selected, y_train, X_val_selected, y_val, train_dataloader, val_dataloader)

        # Step 6: Train Subcategory Models
        print("\n6. Training Benign Subcategory Model...")
        benign_model = SubcategoryModel(num_classes=len(benign_classes))
        benign_model.train_model(benign_train_dataloader, benign_test_dataloader)

        print("\n7. Training Malignant Subcategory Model...")
        malignant_model = SubcategoryModel(num_classes=len(malignant_classes))
        malignant_model.train_model(malignant_train_dataloader, malignant_test_dataloader)
